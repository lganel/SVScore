...
./generateannotations.pl -c 1 -a 2 -b 3 -t 4 -s 5 -e 6 -f 7 tests/dummyannotations.bed
./svscore.pl -f dummyannotations.introns.bed -e dummyannotations.exons.bed -o max,sum,top2,top2weighted,top3weighted,top4weighted,mean,meanweighted -dvc tests/dummyCADD.tsv.gz -p 2 -i tests/stresstest.vcf | grep -v "^#" > tests/stresstest.svscore.test.vcf
echo -e "\n\n"

diff tests/stresstest.svscore.test.vcf tests/stresstest.svscore.noheader.vcf > tests/stresstest.diff
...
./svscore.pl -o max,sum,top5,top10,mean -e refGene.exons.bed -f refGene.introns.bed -dvc $1 -p 2 -i tests/NA12878.sv.vcf | grep -v "^#" | perl -ne 's/(left_|right_)?Intron[^;]*;//g; s/(left_|right_)?ExonTranscript=[^;]*;//g; print' > tests/NA12878.sv.svscore.test.vcf
echo -e "\n\n"

diff tests/NA12878.sv.svscore.test.vcf tests/NA12878.sv.svscore.noheader.vcf > tests/NA12878.diff
...
rm -rf svscoretmp/
rm -f tests/NA12878.sv.svscore.noheader.vcf tests/NA12878.sv.svscore.test.vcf tests/NA12878.diff tests/stresstest.svscore.noheader.vcf tests/stresstest.svscore.test.vcf tests/stresstest.diff dummyannotations.*.bed* refGene* conf.toml
